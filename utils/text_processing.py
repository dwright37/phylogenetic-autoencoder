...
def label_tokens_in_spans(text, spans, in_label="D", out_label="I", label_scheme="Basic"):
    """
    Method to label tokens within a span

    Args:
        text (string):          The text to label
        spans (List[tuple]):    A list of spans within which to label tokens
        in_label (string):      The label to use for tokens within the spans
        out_label (string):     The label to use for tokens outside of the spans
        label_scheme (string):  The labeling scheme to use options are:
                                - Basic: just use the given label
                                - BI (TODO): Use begin/inside labels
    Returns:
    ...
    textptr = 0
    for span in spans:
        I = text[textptr:span[0]].strip()
        labeled.extend([(tok, out_label) for tok in word_tokenize(I)])
        B = text[span[0]:span[1]].strip()
        labeled.extend([(tok, in_label) for tok in word_tokenize(B)])
        textptr = span[1]
